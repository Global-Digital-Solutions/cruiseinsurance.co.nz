...
export default function sitemap(): MetadataRoute.Sitemap {
  const staticPages: MetadataRoute.Sitemap = [
    { url: `${SITE_URL}/`,                        lastModified: NOW, changeFrequency: 'weekly',  priority: 1.00 },
    ...
    { url: `${SITE_URL}/making-a-claim/`,         lastModified: NOW, changeFrequency: 'monthly', priority: 0.80 },
  ...
  // High-value keyword landing pages (priority commercial pages)
  const landingPageEntries: MetadataRoute.Sitemap = landingPages.map(p => ({
    url: `${SITE_URL}/nz-cruise-insurance/${p.slug}/`,
    lastModified: NOW,
  ...
  const blogPostPages: MetadataRoute.Sitemap = blogPosts.map(p => ({
    ...
    changeFrequency: 'monthly' as const,
    priority: p.category === 'Destinations' ? 0.75 : 0.70,
  }));
